...
| B-18a | **Virksomhets-onboarding og admin-API** | Virksomheter opprettes **ikke** automatisk ved første innlogging — de må eksplisitt onboardes. Admin-API (`GET/POST/DELETE /api/v1/admin/virksomheter`) beskyttes av et dedikert Maskinporten-scope (`digdir:kontaktliste.admin`) som tildeles via Digdirs selvbetjeningsløsning (Samarbeidsportalen) eller delegert aktør. Ingen manuell nøkkeldeling. Når Ansattporten er på plass: innlogget brukers orgnr sjekkes mot godkjent liste — avvises med tydelig melding hvis ikke funnet. Avhenger av B-18. |
