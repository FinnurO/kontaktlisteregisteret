...
| # | Funksjon | Beskrivelse |
|---|---|---|
| B-21 | **Frontend-redesign: React + Designsystemet.no** | Erstatt Blazor Server-frontend med React og Digdirs eget komponentbibliotek (Designsystemet.no). All serverlogikk (C#, EF Core, Brreg-integrasjon) beholdes uendret. Bruk **BFF-mønsteret** (Backend for Frontend): .NET-appen håndterer Ansattporten OIDC-sesjonen og setter HttpOnly-cookie, React-klienten kommuniserer bare med BFF — tokens eksponeres aldri i nettleser. **Avhenger av B-02** (Ansattporten) — auth-arkitekturen må fastslås før React-oppsettet. Forutsetter OpenAPI-spec (T-08) for TypeScript-klientgenerering. Designsystemet.no har innebygd Combobox, Select med søk og Checkbox-gruppe — det gjør **B-33 overflødig** hvis B-21 gjennomføres. Gir UX-konsistens med Altinn, Felles datakatalog og øvrige Digdir-produkter. |
| B-33 | **Avansert Brreg-filter med MultiSelect-komponent** | *(Kan droppes hvis B-21 prioriteres — Designsystemet.no dekker behovet.)* Erstatt dagens enkle `<select>`-felter med en gjenbrukbar `<MultiSelect>`-komponent for Blazor: klikk åpner en dropdown med søkefelt (forward-typing), avkrysningsbokser per valg og «Velg alle / Fravelg alle». Brukes på **begge** filterflater: Dynamisk-veiviseren og Søk-og-velg-veiviseren. Nye filterkriterier fra Brreg: **Fylke** (`/enhetsregisteret/api/fylker`, cachet), **Kommune** (`/enhetsregisteret/api/kommuner`, cachet, filtreres på valgt fylke), **Organisasjonsform** (`/enhetsregisteret/api/organisasjonsformer`, cachet — erstatter hardkodet delliste), **Næringskoder** (SSB KLASS, allerede cachet). **Aktivitet** beholdes som post-filter. Fjern «Antall ansatte», «Frivillighet» og «Dato». |
...
| T-05 | SQLitePCLRaw.lib.e_sqlite3 har kjent sårbarhet (transitiv avhengighet fra EF Core 10). Oppgrader til en EF Core/Microsoft.Data.Sqlite-versjon som bruker SQLite 3.50.2 eller nyere når en slik avhengighet er tilgjengelig; adviseringen har foreløpig ingen patchet SQLitePCLRaw-versjon. |
