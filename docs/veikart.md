...
| # | Funksjon | Beskrivelse |
|---|---|---|
| B-33 | **Avansert Brreg-filter med MultiSelect-komponent** | Erstatt dagens enkle `<select>`-felter med en gjenbrukbar `<MultiSelect>`-komponent: klikk åpner en dropdown med søkefelt (forward-typing filtrerer listen), avkrysningsbokser per valg og «Velg alle / Fravelg alle». Brukes på **begge** filterflater: Dynamisk-veiviseren og Søk-og-velg-veiviseren. Nye filterkriterier fra Brreg: **Fylke** (hentes fra `/enhetsregisteret/api/fylker`, cachet), **Kommune** (hentes fra `/enhetsregisteret/api/kommuner`, cachet, filtreres dynamisk basert på valgt fylke), **Organisasjonsform** (hentes fra `/enhetsregisteret/api/organisasjonsformer`, cachet — erstatter dagens hardkodede delliste), **Næringskoder** (fra SSB KLASS — allerede implementert, men vises nå i MultiSelect). **Aktivitet** beholdes som post-filter på resultatlisten. Fjern feltene «Antall ansatte», «Frivillighet» og «Dato». Komponenten bør ligge i `Shared/MultiSelect.razor` med parameter for `Options` (kode+label), `SelectedValues`, `Placeholder` og `OnChange`. |
