...
{
    public static WebApplication MapApiV1(this WebApplication app)
    {
        // ── API v1 ────────────────────────────────────────────────────────────────────
    ...
    internal static object MålgruppeShape(TargetGroup g) => new
    {
        id = g.Id,
        navn = g.Name,
    ...
    private static object KriterierShape(DynamicCriteria c) => new
    {
        orgForm = c.OrgForm,
    ...
    internal static TargetGroupScope? ParseScope(string? scope) => scope switch
    {
...
public record AbonnentRegistrerRequest(string Epost);
public record VirksomhetOnboardRequest(string Orgnr, string Navn, string? OnboardetAv = null);
